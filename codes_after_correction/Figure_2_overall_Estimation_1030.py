...
def Inci2Epi_Para(outbreakData):
    ...
    with basic_model_pop:
        ...
        trace_pop = pm.sample(smpLen, return_inferencedata=False,cores=1,chains = 1);    
...
axes[-1].set_xlabel("k")
axes[-1].set_xticks([0.05,0.1,0.2,0.5,1,2,4])
...
# plt.rc('font', size=20)
fig,ax = plt.subplots(figsize=(16,9),dpi = 500)
fig, axes = joypy.joyplot(posterior_obk_all, ax = ax,by="outbreak", overlap = 0,column=["R_pop",'R_new'],
                          alpha = 0.5,ylim = 'own',x_range = [0.3,8],grid = True,ylabels = False)
